...
- **Mobile narrow-screen still has unfinished polish items**: content clipping and header control overlap are fixed (the sidebar auto-collapses at narrow widths, the header simplifies, overlay hit areas are 44px), but **the send/stop hit areas are still small, the composer has a structural misalignment**, and keyboard avoidance is only a one-line meta fix (**not verified on a real IME**). **Desktop widths are unaffected.**
- **The narrow-screen (phone) layout is still being fixed (historical note, superseded above)**: at narrow widths (roughly 390px and below) the sidebar can clip the main content area and the send button can fall outside the visible area — this repository is fixing that narrow-screen layer; **desktop widths are unaffected**.
...
| **Remote workspaces (SSH / Docker / WSL)** | The code is complete (connect, auth, deploy, handshake and remote session are all present), but the **remote runtime assets are neither shipped with this build nor hosted for CE, and the default CDN has no CE version — so packaged builds cannot use them**. In a source checkout you can point the client at your own static asset root (`ZCODE_REMOTE_ASSET_CDN_BASE_URL`), which we verified end to end. Cause and required layout: [Differences from the official distribution](docs/development/official-diff.md). **How the assets will be supplied is undecided; this row is not a promise to deliver them.**                                                                                                                                                                                                                                                                       |
