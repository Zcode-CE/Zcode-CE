...
| **npm CLI**              | `npx zcode-ce --web`                                 | **Not published to npm yet**: the package name is fixed as `zcode-ce`, but the package does not resolve on the registry (you can confirm with `npm view zcode-ce version`) ⇒ running `npx zcode-ce` today fails. Until it is published, use the Docker form, or build the distribution yourself and run `node bin/zcode.mjs --web`. **The package name and publishing shape are defined in [Headless server distribution](docs/operations/headless-server.md) §6–§7** |
