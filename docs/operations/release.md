...
构建在 CI 上完成，流程、矩阵与常见失败点见 [持续集成与发布构建](./ci.md)。本地打包的入口是：
...
2. **上传失败不阻塞发版**：资产上传是**独立步骤，不是发布门禁**。资产可以事后补传（客户端每次连接都会重新取清单），
   而卡住发版会让已经准备好的安装包多等一个发布周期。判据是"安装包本身能不能用"，不是"托管点此刻是否最新"。
...
1. **口吻**：全文搜一遍主观评价词与口语词（很、非常、优雅、彻底、顺手、搞定、踩坑、实际、！）；
...
| 移除的内容                     | 现在在哪                                                                                             |
| ------------------------------ | ---------------------------------------------------------------------------------------------------- |
| 平台支持矩阵                   | `README.md` 的「安装」一节（桌面端）+ [无头服务器发行包](./headless-server.md) §10（CLI/Web 发行包） |
...
**背景**：上游有时只发**闭源包**、有时更新**开源仓库**。用户需要知道"我们这次在上游那里同步了什么"，
而且必须能区分「**我们做了 A**」与「**上游有 A、我们有意没跟**」—— 否则读者无法判断是漏了还是刻意跳过。

### 位置与命名

- 位置：**「升级须知」之后、「已知限制」之前**（前几节讲"你能感知的变化"，后两节讲"世界状态"；来源说明属于后者之前的过渡）；
- 命名：中文 `## 同步上游`，英文 `## Upstream sync`。
...
| -------- | --------------------------------------------------- | ------------------------------------------------------------------------- |
| 条目     | 能力/修复/协议的短名                                | —                                                                         |
| 上游版本 | 来自哪个上游版本                                    | **只写一次**即可 —— 本版号在标题与页面里已有，不必重复写"上游 X → 我们 Y" |
| 来源     | `开源仓库` / `闭源包`                               | —                                                                         |
| 依据     | `源码比对` / `逆向分析` / `官方 release note`       | **确信度不同**：闭源包时代是逆向推测，有源码后应回头复核                  |
| 同步情况 | `已跟进` / `未跟进（有意）` / `部分跟进` / `不适用` | 见下                                                                      |

**四档的区别（不能混写）**：

- `已跟进`：本版已吸收；
- `未跟进（有意）`：**上游有、我们决定不要或排后** —— 必须给一句原因；
- `部分跟进`：只做了一部分，**必须写清剩下什么**；
- `不适用`：**查过、上游也没有**（例如云 relay、配对服务器、移动壳）—— 这一档是**防止后人重复怀疑**的记录。

### 两条边界

1. **只写对用户有影响的项**。纯内部重构不写进 release notes（留在台账里）。判据用既有的那条：
   **删掉它读者会不会少知道一件可操作或需要知道的事？** 不会就删。
2. `未跟进（有意）` 与 `部分跟进` 里**用户可见的后果**（协议不兼容、入口默认关闭等），
   **同时在「已知限制」里落一条并互相引用** —— 不能让读者只在"同步情况"里看到一半。

### 台账（跨版本累积，不是 release notes）

完整记录（含内部项、依据、以及"确认过上游也不提供"的结论）写在 `docs/development/upstream-sync.md`：
一行一个上游版本，并记录 **fork 点 / 上游最新 tag / 领先落后提交数**（会漂移的值，标快照时刻）。

> **不要**把台账内容复制进 README：README 的「与官方开源版的差异」一节已表达"除差异外一致"，重复会稀释重点。
> **不要**把台账整段搬进 release notes：前者是维护者视图，后者是用户视图。
