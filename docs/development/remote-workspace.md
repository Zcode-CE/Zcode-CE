...
| 场景                                      | 默认指向                                      | 结果                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **本项目发布的安装包**（GitHub Releases） | **社区 CDN** `https://cdn.eidolonmachine.xyz` | ✅ **开箱可用**（前提：该版本已发布到该 CDN；该默认值由发布流水线的仓库变量 `ZCODE_CDN_BASE_URL` 提供，且流水线**要求**它已配置，否则发布 job 直接失败 —— 见 `.github/workflows/release.yml`。**2026-09-24 核实**：该变量的值就是 `https://cdn.eidolonmachine.xyz`，`gh variable list -R Zcode-CE/Zcode-CE`） |
