...
`mc:Ignorable="w14 w15 wp14"`（位置 0）→ `xmlns:mc`（位置 2）⇒ 解析位置 0 时 `mc` 尚未声明 ⇒ 误报 `unbound prefix`。

**规范依据**：Namespaces in XML 1.0 §6.2 —— 命名空间声明在**同一元素内**对该元素的**全部属性**生效，
与属性书写顺序无关。⇒ 是**我们实现过严**，不是文档非法。

**修复**：`readAttributes()` 改为**两遍**——第一遍只做词法校验并收集本元素的 `xmlns` 声明（进作用域），
第二遍才按文档序解析其余属性的前缀。**不是放松检查**：真正未声明的前缀在第二遍同样报 `unbound prefix`。

| 项                     | 值                                                                                                                                                                                                                                                         |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 触发形状               | `<w:document mc:Ignorable="…" … xmlns:mc="…">`（属性顺序是唯一变量）                                                                                                                                                                                       |
| 影响面                 | docx 技能的自身工作流（`docx.cjs` 生成 → `check_office.mjs` 校验）**恒报 fail**；pptx/xlsx 不受影响                                                                                                                                                        |
| 改动文件               | `lib/office-xml-scan.mjs`（两遍解析 + 保留前缀规则）、`lib/office-xml-lex.mjs`（新增 `XMLNS_NAMESPACE_URI` 常量），三插件各一份                                                                                                                            |
| 同时补上的两条既有缺口 | ① 同一元素内**重复声明同一前缀**（expat 报 `duplicate attribute`）；② 保留前缀 `xmlns` 被声明（expat 报 `reserved prefix (xmlns)…`）。两条原先都**静默放行非法输入** —— 因为单遍解析下「后声明覆盖前声明」，缺陷被掩盖；改两遍后若不补，反而会变成真的放行 |
...
**没有一个含 `mc:Ignorable`**（`grep -c Ignorable` = 0），也**没有一个来自真实办公库产物** ⇒ 缺陷落在盲区里。
本次新增的 `checkOfficePrefix.test.mjs` 专门用**真实库产物**打这个盲区。

**为什么用插件自己的 `docx` 依赖而不是 `scripts/office-node/docx.cjs`**：后者是**构建产物**
（stage 到 `cli/dist/`、被 `.gitignore` 忽略），干净检出上不存在；`docx@9.7.1` 是本插件
`package.json` 里声明的同一个库，两者同源同版本，测试里用它才能在任何检出上跑。
