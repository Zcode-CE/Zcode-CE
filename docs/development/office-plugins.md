...
对任何「读不可信文件」的校验器，只把 `MAX_*` 常量写对**不足以**让峰值内存受约束。这条判据来自 OFFICE-5 的实测：第一版 JS 重写**完整保留**了 4 个 `MAX_*` 与分块解析，峰值 RSS 仍是 Python 版的 5 倍（同一夹具：dense 1182 MiB vs 239 MiB；160 MB 媒体夹具 381 MiB vs 21 MiB）。两个根因都**不在常量上**：
...
1. **峰值内存回归测试**：对元素密集夹具断言峰值 RSS 上限。OFFICE-5 的写法见 `documents-plugin/test/checkOffice.test.mjs` 的「元素密集输入的峰值内存受元素预算约束」用例（阈值 900 MiB，实测回归值 1182 MiB，修好后 329 MiB）。
...
2. **与基准实现的峰值对比**：保留一份基准（本目录就是 `.py`）并实测同一夹具的峰值 RSS，量级差距要能解释（V8 基线约 50 MiB 属正常）。
