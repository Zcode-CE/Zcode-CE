...
> 实现落点与验证命令见下文「实现与验证」一节。
> 状态：**已实施**（MCP 单工具启停；插件内单工具启停仍未提供）。
> 范围：**MCP 子工具级启停**（以下简称「工具启停」）。插件整包启停、危险命令清单的既有规则不变。
...
6. **远端 workspace**：配置经同一个 `mcp.servers` 形状下发，但**未做远端端到端实测**。

---

## 8. 与既有文档的关系

- 资源启停（MCP/插件）的所有者是**各资源页**，本文落实为**该 server 配置对象内的 `disabledTools`**；「资源页能表达什么」从整包 `enabled` 精确到工具级。
- 本文是「资源页表达工具级启停」这一项的落地：资源页本身仍不提供整包之外的开关键，两者不冲突。
