...
| **远程工作区（SSH / Docker / WSL）** | 代码完整（连接、鉴权、部署、握手、远端会话都在），**但远程运行时资源不随包分发**：默认指向的官方 CDN 只有官方版本号，**打包版默认不可用**。本版提供自托管三步走（生成 → 装配 → 端到端自检）与[远程工作区文档](docs/development/remote-workspace.md)，把客户端指向你自己的发布根即可用；本项目不自建托管，**因此不作「开箱即用」的承诺**                                                                                                                                                                          |
