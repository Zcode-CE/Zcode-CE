...
| **无头服务器 + 浏览器面板** | 把「服务端 + 浏览器面板」作为一条独立使用路径：可跑在**没有桌面环境的机器**上，用同一局域网/私网内的浏览器（**含手机窄屏**）操作**这台机器上的工作台**。**它不接管桌面端**：不是接管桌面端正在开的会话，桌面端已连的远端 SSH / Docker 目标**不会**共享给浏览器。**默认只监听回环 `127.0.0.1`**；要对外监听**必须**同时给令牌，否则拒绝启动。自托管、**不经任何中继**、不采集遥测；不是官方那套「扫码 + 云 relay + 移动壳」（那套未随源码分发）。见[网页远控](docs/development/web-remote-control.md) |
...
- **桌面自动化（Computer Use）**：Windows 为正式支持；**Linux 为实验性**（默认关闭，需在设置中开启）。官方发行版不支持 Linux 桌面自动化，本版通过开源实现提供，但受上游验证范围限制。详见[桌面自动化文档](docs/development/computer-use.md)。
- **桌面自动化在 Linux 上有环境限制**：已在 X11 / Sway / KDE Wayland 验证，**Wayland 下截图不可用**（元素操作不受影响），GNOME 未完整验证。
...
- **手机窄屏仍有未完成的打磨项**：主内容区裁切与头部控件重叠已修（窄屏默认收起侧栏、头部按窄屏简化、浮层命中区补到 44px），仍有三项未完成：**发送/停止键命中区偏小、composer 存在结构错位**、软键盘避让只做了单行 meta 修复（**真机 IME 未验证**）。**桌面宽度不受影响。**
...
| **远程工作区（SSH / Docker / WSL）** | 代码完整（连接、鉴权、部署、握手、远端会话都在），**但远程运行时资源不随包分发**。本项目已投运**社区 CDN**（`https://cdn.eidolonmachine.xyz`，自建对象存储 + 自定义域名，**不中继、不采集**）并托管了各版本资产：**我们发布的安装包默认指向它、开箱可用**（该版本未托管时需自行指向发布点）。**Docker 已在真 Docker 29.8.0 / rootless / cgroup v2 上实测通过；WSL 未实测**（需 Windows 客户端）。自建者可在**桌面端**设置页填自定义 CDN 地址、或设 `ZCODE_REMOTE_ASSET_CDN_BASE_URL`（**Web / CLI 只能用这个环境变量**），也可按[远程工作区文档](docs/development/remote-workspace.md)自建三步走（**远端需 POSIX shell：Linux / macOS 的 x64 / arm64；Windows 远端被拒绝，32 位 ARM、ppc64le/s390x、BSD 无资产，Alpine(musl) 实测不可用**） |
...
| **npm CLI**            | `npx zcode-ce --web`                 | **已发布**：`npm view zcode-ce version` ⇒ `3.14.3-ce.3`。包名 `zcode-ce`，`dist-tags` 的 `latest` 与 `next` 均指向该版本 ⇒ `npx zcode-ce` 可直接用。包体较大（解包约 293 MB、11864 个文件，含随包发布的原生依赖），首次 `npx` 需先下载解包，请预留时间。**包名与发布形态以[无头服务器发行包](docs/operations/headless-server.md) §6–§7 为准** |
...
| 无头服务器 + Web 面板 | 无桌面环境也可用：`zcode --web` 起服务，浏览器（含手机）打开面板操作这台机器上的工作台 | `pnpm dev:web`（开发）；`pnpm build:zcode` 产出可独立部署的包 |
