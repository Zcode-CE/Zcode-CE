 ...
 * ## 默认状态 = 攻击面为零
 *
 * 未启用任何 webhook 渠道时，gate 在**第一个判断**就返回 c.notFound()：**不读配置、不读凭据、
 * 不读请求体、零 I/O**，其响应与「这条路由根本不存在」**逐字节相同**（实测 spec §8.4 R3/R15）
 * ⇒ 「未注册」与「注册了但拒绝」在可观测层面不可区分。
 *
 * ## 安全边界在哪一层（不要误读）
 *
 * options.enabled 只是**快速短路**（省掉未启用时的全部工作），**不承担安全边界**。
 * 真正的边界是**逐请求**的：③ 运行期判定 + ④ 凭据校验 + ⑤ 限流。
 ...
 * /bot/** 的请求体上限（spec §7.1）。
 *
 * 取值依据：入站附件有一条 dataBase64 通道**走请求体**（botsService 的 resolveAttachmentBytes），
 ...
 * 的 length !== length || !timingSafeEqual(...)）。timingSafeEqual 在长度不等时**抛错**，
 * 故长度检查必须前置。
 *
 * 诚实边界：只保证「用了正确的原语」，**不能**证明端到端无时序侧信道。
 */
/**
 * 有界流式读请求体：超过 `maxBytes` 立即中止并返回 null。
 *
 * 为什么不用 `hono/body-limit`：它的机制是「把 body 换成计数流 → 调 next() → 若下游读了体
 * 并触发 BodyLimitError，则在 next() 之后把 c.res 换成 413」。本 gate 是**自行终结**请求的
 * （不把请求交给下游 handler），因此 bodyLimit 的错误永远不会浮现 —— 实测：把 33MB 打进来
 * 仍返回 200（上限**完全失效**）。⇒ 改为在 gate 内自己读，顺序与上限都完全可控。
 *
 * 性质（与 spec §7.1 的承诺一致）：
...
export interface BotIngressGateOptions {
   ...
   */
  enabled: boolean;
  /** bot 面**独立**限流桶（spec §4.2：与令牌面共享会造出跨面拒绝服务）。 */
 ...
 *
 * **必须注册在令牌中间件之前**，且**无条件注册**（「是否启用」是 gate 内部的第一个判断）。
 */
export function createBotIngressGate(
  ...
  // 注意签名里**不接 next**：本 gate 对 /bot/** 一律自行终结请求（永不把请求交给下游）。
  // 这不是省参数 —— 它是 D1 的结构性保证的代码形态：没有 next 就**不可能**意外放行到
  ...
  return async (c) => {
    ...
    if (!isBotProvider(parsed.provider) || parsed.provider !== "webhook") {
      // 400：调用方错误、不可重试。只回显**入参** provider，**不回显**本机支持的 provider 列表
      // （否则等于免费告诉攻击者该试探什么）。
    ...
    if (!parsed.botId) {
      ...
      // （secret 逐 bot），且「唯一」会随配置条数**静默变化** —— 上游的该兜底正是
    ...
    //    顺序理由：被封禁的地址应当在**做任何校验之前**就被拒 —— 否则「已封禁」的语义会退化成
    ...
    // ⑦ 有界读体（上限见 BOT_INGRESS_MAX_BODY_BYTES）：在**凭据校验之后**读，
    ...
    try {
      result = await botsService.handleProviderCallbackResponse("webhook", {
        ...
        botId: parsed.botId,
        rawBody: rawBodyText,
        // secret 继续往下传：服务层还有一层**独立的** fail-closed 校验（纵深，spec §3.7）。
