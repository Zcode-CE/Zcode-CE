...
export function createTelegramChannelRuntime(deps: TelegramChannelRuntimeDeps) {
  ...
  async function reconcile(
    ...
    // Modified by ZCode: R1 修复 —— 先取「停轮询之前」的活跃快照。下面停轮询会清空 runtimes，
    // 而禁用分支需要知道该 bot 本次是否真的从启用态转过来（决定要不要清一次命令菜单）。
    const previouslyActiveTelegramIds = new Set(runtimes.keys());
    ...
    for (const bot of currentConfig.bots) {
      if (bot.provider === "telegram" && bot.enabled && bot.credentialRef) {
        ...
        // Modified by ZCode: R1 修复 —— 上游在这里无条件 syncCommands(bot)，而 :76-78 的注释
        // 写的是「删除或禁用 bot 时也不能为了清命令访问第三方 API」—— 那个 return 只在
        // runBackgroundTasks === false 时生效，本地 host 上已禁用的 bot 每次 reconcile
        // 都会向 api.telegram.org 发一次 deleteMyCommands，与注释承诺不符。
        //
        // 修复依据：清命令菜单只在「本次运行内由启用变为禁用」这一次有意义。
        // previouslyActive 是上面停轮询之前取的快照，因此：
        //   · 启用 → 禁用：命中一次，清一次（用户可感知的正确行为得以保留）；
        //   · 一直是禁用（含进程重启后）：不命中，完全不出网。
        if (previouslyActiveTelegramIds.has(bot.id)) {
          await syncCommands(bot);
        }
        deps.statusSink.setRuntimeStatus({
          botId: bot.id,
