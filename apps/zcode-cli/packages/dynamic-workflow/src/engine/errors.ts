 ...
 *   而不是 run 级，靠的是拒绝通道而不是严重程度：世界读取返回一个脚本能 `catch` 的
 *   promise，所以"缩窄 pattern 或加个 glob"是一条脚本真能走的路。也刻意不折进
 *   DriverError——上限是脚本可以据以重写自己的契约，而靠匹配 message 文本区分两者，
 *   正是这个联合类型存在的目的所要防的。
 * - run 级：InputHashMismatch / UnknownActor / MissingAskSpec /
 *   DuplicateActorName——同一个 run 内两次 createActor 得到相同的非空有效名
 ...
 *   分界同理、结论相反：`report` 返回 `void`，脚本没有可以 catch 的通道，除了 run 无处可放。
...
export type WorkflowErrorCode =
  ...
  | "ProviderStop"
  // ——————————— 用户面产物———————————
  // ⚠ 术语：这一批 artifact 全是用户面产物（脚本发布给用户看的产出），与
  // `RunSettlement.artifact`（顶层返回值）无关。
  //
  // 通道按成员族分裂，与 WorldReadCapExceeded / ReportCapExceeded 的分界同一条论证：
