...
export async function createZCodeApp(options: ZCodeAppOptions): Promise<ZCodeApp> {
  ...
  // 曾经在这里 createModelTelemetry(...) 造进程级遥测 Owner，并向 scriptWorkflowFacade /
  // script-workflow child / AgentRuntime / workflowFacade 四处注入 agentTelemetry 与 statusSink。
  // 遥测删除后不再有 Owner：core 的 RuntimeTelemetryFacade 在 port 缺席时退化为
  // NOOP_AGENT_EXECUTION_TELEMETRY（core/src/telemetry/runtime-telemetry.ts:46），
  // 所以下面各处**不再传** agentTelemetry，由 core 自行兜底。
    ...
    return {
      ...
      close: async () => {
        try {
          ...
          // 遥测 Owner 的 shutdown 已随 telemetry 包删除；dispose 仍必须无条件执行。
